...
1. **Contract-test the cross-repo math instead of porting it.** ✅ Shipped — `npm run golden-vectors` records 125 cases from the web modules into `public/golden-vectors.json`; `gpx/tests/goldenVectors.test.ts` replays every one through the API's copies, with coverage enforced from the module exports so a ported function without a vector fails. The line-level divergence turned out to be structural rather than numeric — every top-level case agreed immediately — but extending the vectors down to the helpers found a real defect: `calculateWeatherAdjustment` compared `paceUnit === "miles"` where the web lower-cases first, so a caller using the site's own vocabulary (`PaceUnit` is `"km" | "Miles"`) got the km adjustment applied to a mile pace — 8:00-8:30 on the site, 7:49-8:19 from the API. Latent rather than live (the MCP tool's zod enum is `["km","miles"]`), and fixed in three places.
