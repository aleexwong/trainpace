 ...
 */
export async function migrateBookmarksToRouteKeys(dryRun: boolean = true): Promise<void> {
  ...
  try {
    ...
    snapshot.forEach((docSnapshot) => {
      ...
      if ((!bookmark.routeKey && bookmark.routeSlug) || 
    ...
    for (const bookmark of bookmarksToMigrate) {
      try {
        const routeKey = bookmark.routeKey || migrateSlugToRouteKey(bookmark.routeSlug || '');
        
        if (!routeKey) {
          console.warn(`Could not migrate bookmark ${bookmark.id}: no valid route key found for slug "${bookmark.routeSlug}"`);
          errorCount++;
          continue;
        }
        ...
        console.log(`${dryRun ? '[DRY RUN] ' : ''}Migrating bookmark ${bookmark.id}:`);
        console.log(`  routeSlug: "${bookmark.routeSlug}" → routeKey: "${routeKey}"`);
