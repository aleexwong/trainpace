      ...
      <div className="relative">
        ...
        <Marquee pauseOnHover speed="slow" className="[--duration:40s]">
          {features.map((item, idx) => (
            <MarqueeItem key={idx}>
              <div className="mx-4 flex items-center gap-2 px-6 py-3 bg-background rounded-full shadow-sm border border-blue-200 hover:shadow-md transition-shadow">
          ...
          {marathonRoutes.map((route, idx) => (
            <MarqueeItem key={idx}>
              <Link to={`/preview-route/${route.slug}`}>
                <div className="mx-4 flex items-center gap-2 px-6 py-3 bg-background rounded-full shadow-sm border border-green-200 hover:shadow-md hover:border-green-300 transition-all cursor-pointer">
