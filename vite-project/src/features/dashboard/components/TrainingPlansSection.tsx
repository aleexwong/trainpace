...
export function TrainingPlansSection({
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 mb-6">
          Create your first personalized training plan for your next race.
        </p>
        <a
  ...
  // Pagination logic
  const totalPages = Math.ceil(trainingPlans.length / PLANS_PER_PAGE);
  const startIndex = (currentPage - 1) * PLANS_PER_PAGE;
  const endIndex = startIndex + PLANS_PER_PAGE;
  const currentPlans = trainingPlans.slice(startIndex, endIndex);
    ...
    <>
      <div className="flex justify-between items-center mb-6">
        <div className="text-sm text-gray-600">
          {trainingPlans.length} training plan
          {trainingPlans.length !== 1 ? "s" : ""} saved
          {totalPages > 1 && (
