  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center">
        <div
          className={`w-1/2 text-center text-xs font-semibold transition-colors ${
            paceUnit === "km" ? "text-white" : "text-indigo-700"
          }`}
        >
          min/km
        </div>
        <div
          className={`w-1/2 text-center text-xs font-semibold transition-colors ${
            paceUnit === "mi" ? "text-white" : "text-indigo-700"
          }`}
        >
          min/mi
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Daniels&apos; 5 zones for VDOT {vdot}
          </p>
        </div>
        <PaceUnitToggle paceUnit={paceUnit} onToggle={onTogglePaceUnit} />
      </div>
          ...
          return (
            ...
            >
              ...
              >
                {/* Zone badge */}
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${colors.badge}`}
                >
                  {zone.shortName}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">{zone.intensityRange} VO&#8322;max</span>
                  </div>
                ...
                <div className="text-right shrink-0">
                  <p className={`text-lg sm:text-xl font-bold font-mono ${colors.text}`}>
              ...
              {isExpanded && (
                <div className={`px-4 pb-4 border-t ${colors.border}`}>
                  <p className="text-sm text-gray-600 mt-3 mb-3 sm:hidden">
                    {zone.description}
                  </p>
                  <div className="space-y-2">
                    {workouts.map((w, i) => (
                      <div
                        key={i}
                        className={`flex items-start gap-2 p-2.5 rounded-lg ${colors.bg}`}
                      >
                        <Info className={`w-4 h-4 mt-0.5 shrink-0 ${colors.text}`} />
                        <div>
                          <p className="text-sm font-medium text-gray-900">{w.workout}</p>
