...
### Next
- **~~Strava / Garmin import~~ — BLOCKED, do not start.** Was listed as the biggest single UX unlock. Re-checked against the primary sources in 2026 and it is not available to us: Strava's [API Policy](https://www.strava.com/legal/api_policy) forbids using their data "in connection with the development, training, evaluation, or operation of any AI Application" and separately bans operating an MCP server over it — which is precisely what `/import`, the fuel planner and `mcp/health-server.ts` do — while the Standard Tier caps at 10 athletes, requires a paid developer subscription, and permits only 7 days of caching (so no Firestore-backed dashboard either). Garmin's Connect Developer Program is on hold and issuing no new developer accounts. Revisit only if those terms change.
  - Consequence for `/import` (Apple Health, shipped): the manual export is not a stopgap, it is the route. The Health export can't be scheduled, and Apple Shortcuts can't read workouts at all — `Find Health Samples` has no "Workout"/"Run" type, only quantity samples — so per-run pace cannot be automated for free either. See `vite-project/docs/apple-health.md`.
