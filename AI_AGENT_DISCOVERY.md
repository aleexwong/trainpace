...
- **Static answers in JS-only pages** — `/`, `/calculator`, `/vdot`, `/plan`, and `/fuel` prerender real reference tables (training paces by 5K time, equivalent race times by VDOT, carb/gel targets by finish time, plan phase structure) computed at build time from the same math modules the widgets use. A non-JS reader gets quotable numbers instead of an empty shell. The tables are deliberately **hub-only**: the ~60 programmatic pace pages and ~20 fuel pages link to them rather than each carrying a copy, so template-generated pages don't become more near-duplicate than they already are.
...
- Reference tables belong on hub pages only. To add one to a new tool page, call it from that page's block function in `page-docs.ts`; to point a programmatic page at it, add an entry to `REFERENCE_HUBS`. Do not inline tables into `seoPageBlocks()`.
- The canonical host is `https://trainpace.com` — **no `www`**. It comes from `BASE_URL` in `src/lib/seo/types.ts`; import that constant rather than hardcoding a host. Both hosts currently answer on 200 with no redirect, so the canonical tag is the only thing disambiguating them: a hardcoded `www` URL anywhere silently splits ranking signals for that page.
